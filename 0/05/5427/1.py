from collections import deque
from sys import stdin
input = stdin.readline

BLANK, WALL, S, FIRE, EXIT = '.#@* '
DIRS = [(1, 0), (0, 1), (-1, 0), (0, -1)]

def build(w, h, structure):
  building = [[EXIT] * (w + 2) for _ in range(h + 2)]
  coords = { S: (0, 0), FIRE: [] }

  for r in range(h):
    for c in range(w):
      cell = structure[r][c]
      if cell == S: coords[S] = (r + 1, c + 1)
      elif cell == FIRE: coords[FIRE].append((r + 1, c + 1))

      building[r + 1][c + 1] = structure[r][c]

  return building, coords


def test(w, h, structure):
  building, coords = build(w, h, structure)
  
  dq = deque([*coords[FIRE], coords[S]])
  
  while dq:
    r, c = dq.popleft()
    curr = building[r][c]
    if curr[0] not in [S, FIRE]: continue

    for dr, dc in DIRS:
      nr, nc = r + dr, c + dc
      if not (0 <= nr < h + 2 and 0 <= nc < w + 2): continue
      next = building[nr][nc]

      if next == WALL: continue

      elif next == EXIT:
        if curr[0] == S: return len(curr)
        continue
      
      elif next == BLANK:
        building[nr][nc] = curr + curr[0]

      elif curr[0] == S:
        if next[0] == FIRE: continue
        if next[0] == S: continue

      elif curr[0] == FIRE:
        if next[0] == FIRE: continue

      if (nr, nc) in dq: continue 
      dq.append((nr, nc))

  return 'IMPOSSIBLE'

for _ in range(int(input())):
  w, h = map(int, input().split())
  structure = [[*input().strip()] for _ in range(h)]
  print(test(w, h, structure))