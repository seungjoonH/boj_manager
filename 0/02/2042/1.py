...
def build(index=0, left=0, right=n - 1):
  if left == right: segment[index] = numbers[left]; return
...
def adjust(i, delta, index=0, left=0, right=n - 1):
  ...
  if i in range(left, mid + 1): adjust(i, delta, index * 2 + 1, left, mid)
  if i in range(mid + 1, right + 1): adjust(i, delta, index * 2 + 2, mid + 1, right)
